...
struct _Graphic_engine
{
  ...
  Area *dialogue; /*!< Area where the dialogue is printed */
};
...
Graphic_engine *graphic_engine_create()
{
  ...
  ge->dialogue = screen_area_init(80, 8, 29, 11);
  return ge;
}
...
void graphic_engine_paint_game(Graphic_engine *ge, Game *game)
{
  ...
  if ((id_act = game_get_player_location(game)) != NO_ID)
  {
    ...
    if ((light = space_get_light(space_act)) == FALSE) {
      for (i = 0; i < MAX_OBJECTS && (aux_obj = game_get_object(game, game_get_object_id_at(game, i))) != NULL; i++) {
        if (object_can_illuminate(aux_obj) && object_is_turnedon(aux_obj)) {
          light = TRUE;
          break;
        }
      }
    ...
    if (id_back != NO_ID)
    {
      ...
      screen_area_puts(ge->map, str);
      sprintf(str, "                |%5s %4s %4s|", space_get_light(game_get_space(game, id_next)) ? obj0 : " ",
              space_get_light(game_get_space(game, id_next)) ? obj1 : " ",
              space_get_light(game_get_space(game, id_next)) ? obj2 : " ");
      screen_area_puts(ge->map, str);
      sprintf(str, "                +---------------+");
      screen_area_puts(ge->map, str);
    ...
    if (id_act != NO_ID && id_right != NO_ID && id_left != NO_ID)
    {
      ...
      screen_area_puts(ge->map, str);
      sprintf(str, "            |   |%5.5s %4.4s %4.4s|   |           ", light ? obj0 : " ", light ? obj1 : " ", light ? obj2 : " ");
      screen_area_puts(ge->map, str);
      sprintf(str, "------------+   +---------------+   +-----------");
      screen_area_puts(ge->map, str);
    }
    ...
    else
    {
     ...
     if (id_next == game_get_enemy_location(game) && enemy_get_health(game_get_enemy(game)) > 0)
      strcpy(enemy, " ( ·_· )");
...
screen_area_clear(ge->descript);
  ...
  screen_area_puts(ge->descript, str);
  for (i = 0; i < MAX_OBJECTS && (aux_obj = game_get_object(game, game_get_object_id_at(game, i))) != NULL; i++) {
    if (!player_has_object(game_get_player(game), object_get_id(aux_obj))) {
      if ( (game_get_object_location(game, object_get_id(aux_obj)) == id_act && light) || space_get_light(game_get_space(game, game_get_object_location(game, object_get_id(aux_obj)))) ) {
  ...
  sprintf(str, " ");
  screen_area_puts(ge->descript, str);
  sprintf(str, "  Space: %s", space_get_sdesc(space_act));
  screen_area_puts(ge->descript, str);
  sprintf(str, " ");
  screen_area_puts(ge->descript, str);
  if (command_get_type(last_cmd) == INSPECT && command_get_status(last_cmd) == OK) {
    if (light == TRUE) {
      if (!strcasecmp("s", command_get_arg(last_cmd, 0)) || !strcasecmp("space", command_get_arg(last_cmd, 0))) {
        sprintf(str, "  Description (space): ");
        screen_area_puts(ge->descript, str);
        sprintf(str, "   %s", space_get_ldesc(space_act));
        screen_area_puts(ge->descript, str);
      } else {
        sprintf(str, "  Description (%s): ",
                object_get_name(game_get_object_by_name(game, command_get_arg(last_cmd, 0))));
        screen_area_puts(ge->descript, str);
        sprintf(str, "   %-18s", object_get_descr(game_get_object_by_name(game, command_get_arg(last_cmd, 0))));
        screen_area_puts(ge->descript, str);
      }
    } else {
      sprintf(str, " Lights are off.");
      screen_area_puts(ge->descript, str);
    }
  }
  
  /* Paint in the banner area */
  screen_area_puts(ge->banner, "The mysterious castle");

  /* Paint in the help area */
  screen_area_clear(ge->help);
  sprintf(str, " The commands you can use are:");
  screen_area_puts(ge->help, str);
  sprintf(str, "   take t, drop d, attack a, move m, inspect i, open o,");
  screen_area_puts(ge->help, str);
  sprintf(str, "       exit e, turnon tn or turnoff tf");
  screen_area_puts(ge->help, str);
 ...
 screen_area_clear(ge->dialogue);
  if (dialogue_get_type(game_get_dialogue(game)) == ERROR_MSG) {
    sprintf(str, " >:[ %s", dialogue_get_message(game_get_dialogue(game)));
  } else if (dialogue_get_type(game_get_dialogue(game)) == SUCCESS_MSG) {
    sprintf(str, " %s", dialogue_get_message(game_get_dialogue(game)));
  } else {
    /* Welcome message */
    sprintf(str, " Welcome!!! You can start playing");
    screen_area_puts(ge->dialogue, str);
    sprintf(str, " GOOD LUCK !!!");
    screen_area_puts(ge->dialogue, str);
    sprintf(str, " ");
    screen_area_puts(ge->dialogue, str);
    sprintf(str, " The goal of the game is to escape from the ghost");
    screen_area_puts(ge->dialogue, str);
    sprintf(str, " But be careful, brave explorer.");
    screen_area_puts(ge->dialogue, str);
    sprintf(str, " ");
  }
  screen_area_puts(ge->dialogue, str);
