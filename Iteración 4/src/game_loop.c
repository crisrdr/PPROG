...
while ((command_get_type(cmd) != EXIT) && !game_is_over(game)) /* The game is in loop while the user doesn't enter the EXIT cmd or the game isn't over */
  {
    graphic_engine_paint_game(gengine, game);
    cmd = command_get_user_input();
    game_update(game, cmd);
         
    cmd_name = cmd_to_str[command_get_type(cmd) - NO_CMD][CMDL]; /* = get the long name of the entered command */

    if (log_file) {
      fprintf(log_file, "%s", cmd_name);
      for (i = 0; i < MAX_ARG; i++) {
        cmd_arg = command_get_arg(cmd, i);
        if (strlen(cmd_arg) > 0)
          fprintf(log_file, " %s", cmd_arg);
        else
          break;
      }
      fprintf(log_file, ": %s\n", command_get_status(cmd) == OK ? "OK" : "ERROR");
    }
  }
}
void game_loop_cleanup(Game *game, Graphic_engine *gengine)
