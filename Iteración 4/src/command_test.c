...
void test1_command_set_arg(){
  Command *cmd = NULL;
  cmd = command_create();
  PRINT_TEST_RESULT(command_set_arg(cmd, "Take", 3) == OK);
  command_destroy(cmd);
}
...
void test2_command_set_arg(){
  Command *cmd = NULL;
  cmd = command_create();
  PRINT_TEST_RESULT(command_set_arg(cmd, NULL, 3) == ERROR);
  command_destroy(cmd);
}
...
void test1_command_get_arg(){
  Command *cmd = NULL;
  cmd = command_create();
  command_set_arg(cmd, "adios", 2);
  PRINT_TEST_RESULT(strcmp(command_get_arg(cmd, 2), "adios") == 0);
  command_destroy(cmd);
}

void test2_command_get_arg(){
  Command *cmd = NULL;
  PRINT_TEST_RESULT(command_get_arg(cmd, 1) == NULL);
}

void test1_command_set_type(){
  Command *cmd = NULL;
  PRINT_TEST_RESULT(command_set_type(cmd, MOVE) == ERROR);
}

void test2_command_set_type(){
  Command *cmd = NULL;
  cmd = command_create();
  PRINT_TEST_RESULT(command_set_type(cmd, MOVE) == OK);
  command_destroy(cmd);
}
...
void test3_command_get_type(){
  Command *cmd = NULL;
  cmd = command_create();
  command_set_type(cmd, MOVE);
  PRINT_TEST_RESULT(command_get_type(cmd) == MOVE);
  command_destroy(cmd);
}
